...
class tv_display_label extends javax.swing.JLabel
{
	public tv_display_label()
	{
		super(main.create_image_icon("phone.png", "phone"));
		m_text_message="";
	}
...
class main implements Runnable
{
		...
		Exception
	{
		DeviceIdentity identity = new DeviceIdentity(
				UDN.uniqueSystemIdentifier("Demo Phone"+Math.random()));

		DeviceType type=new UDADeviceType("phone", 1);
		...
		LocalService<display> display_service=
			(LocalService<display>)
			new AnnotationLocalServiceBinder().read(display.class);

		m_manager=new DefaultServiceManager<display>(display_service,
				display.class);
