...
class rain_display_label extends javax.swing.JLabel implements MouseListener
{
	public rain_display_label()
	{
		super(main.create_image_icon("rain_gauge.png", "Rain Gauge"));
	...
	public void click(){
		main.m_manager.getImplementation().addRain(plus_minus);
	}
...
{
	static DefaultServiceManager<Rain_gauge> m_manager;
	static rain_display_label m_rain_display_label;
		...
		Exception
	{
		DeviceIdentity identity = new DeviceIdentity(
				UDN.uniqueSystemIdentifier("Demo Rain gauge"));
		...
		LocalService<Rain_gauge> display_service=
			(LocalService<Rain_gauge>)
			new AnnotationLocalServiceBinder().read(Rain_gauge.class);

		m_manager=new DefaultServiceManager<Rain_gauge>(display_service,
				Rain_gauge.class);
	...
	public static void main(String[] args)
	{
		...
		JFrame frame=new JFrame("Rain Gauge");
		m_rain_display_label=new rain_display_label();
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);

		frame.getContentPane().setBackground(java.awt.Color.white);
		frame.getContentPane().add(m_rain_display_label, BorderLayout.CENTER);
	...
	public void run()
	{
		try {
			...
			int raining = 0;
			m_manager.getImplementation().setRaining(raining);
